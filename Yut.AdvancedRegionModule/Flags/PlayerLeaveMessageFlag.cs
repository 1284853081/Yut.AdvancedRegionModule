...
{
    internal class PlayerLeaveMessageFlag : RegionFlag<PlayerLeaveMessageConfig>
    {
        protected override void OnPlayerLeave(Player player)
        {
            ...
            switch (mode)
            {
                ...
                case EBroadcastMode.Area:
                    ChatManager.serverSendMessage(message, Palette.SERVER, null, player?.channel.owner, EChatMode.LOCAL, null, rich);
                    break;
                case EBroadcastMode.Group:
                    ChatManager.serverSendMessage(message, Palette.SERVER, null, player?.channel.owner, EChatMode.GROUP, null, rich);
                    break;
            }
        }
    }
