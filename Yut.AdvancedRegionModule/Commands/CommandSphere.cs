...
{
    internal sealed class CommandSphere : IRocketCommand
    {
        public AllowedCaller AllowedCaller => AllowedCaller.Player;
        public string Name => Keys.SPHERE;
        ...
        public void Execute(IRocketPlayer caller, string[] command)
        {
            ...
            if (command.Length == 3)
            {
                ...
                {
                    if (isStatic)
                        CommandStatic.Command.ExecuteParams(caller, command[1], Keys.SPHERE, command[2]);
                    else
                        CommandDynamic.Command.ExecuteParams(caller, command[1], Keys.SPHERE, command[2]);
                }
