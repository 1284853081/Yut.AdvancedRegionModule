...
{
    public sealed class CommandStatic : IRocketCommand
    {
        private static CommandStatic command;
        public static CommandStatic Command
        {
            get
            {
                if (command == null)
                    command = new CommandStatic();
                return command;
            }
        }
        public AllowedCaller AllowedCaller => AllowedCaller.Player;
        public string Name => Keys.COMMAN_STATIC;
        ...
        public void Execute(IRocketPlayer caller, string[] command)
        {
            if (command.Length == 1)
            {
                ...
                {
                    var regions = RegionManager.Instance.StaticRegionTypes;
                    var count = regions.Count;
                    for (int i = 0; i < count; i++)
                        UnturnedChat.Say(caller, Yut.Instance.Translate(Keys.KEY_STATIC_REGION_TYPES, regions[i]));
                }
                else if (command[0] == Keys.REGIONS)
                {
                    var regions = RegionManager.Instance.StaticRegions;
                    var count = regions.Count;
                    for (int i = 0; i < count; i++)
                    {
                ...
                {
                    var flags = FlagManager.Instance.StaticFlagTypes;
                    var count = flags.Count;
                    for (int i = 0; i < count; i++)
                        UnturnedChat.Say(caller, Yut.Instance.Translate(Keys.KEY_STATIC_FLAG_TYPES, flags[i]));
                }
                else
            ...
            else if (command.Length == 2)
            {
                ...
                {
                    var region = RegionManager.Instance.TryFindRegion(command[1], true);
                    if (region is null)
                    {
                        UnturnedChat.Say(caller, Yut.Instance.Translate(Keys.KEY_REGION_NOT_FOUND, command[1]));
                        return;
                    }
                    var result = RegionManager.Instance.DestroyStaticRegion(region);
                    switch (result)
                    {
            ...
            else if (command.Length == 3)
            {
                ...
                {
                    var result = RegionManager.Instance.CreateStaticRegion(command[1], command[2], out var region);
                    switch (result)
                    {
